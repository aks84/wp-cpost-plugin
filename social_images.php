<?php
/**
 * Plugin Name:     social_images
 * Plugin URI:      https://amulyashahi.org/demo-plugin
 * Description:     social_images custom post type plugin
 * Author:          Amulya
 * Author URI:      https://amulyashahi.com
 * Text Domain:     social_images
...
function social_img_init() {
    register_post_type( 'social_img', array(
        'labels'                => array(
            ...
            'menu_name'             => __( 'Social Images', 'social_images' ),
        ),
        'public'                => true,
        'hierarchical'          => false,
        'show_ui'               => true,
...
function add_link_meta_boxes( $social_img ) {
     add_meta_box( 
        'social-img-meta-box',
        __( 'Social Links' ),
        'social_links_input_ui',
        'social_img',
        'normal',
        'default'
    );
...
function social_links_input_ui($social_img){
    ?> 
    <div class="social_links">
        <h2>Paste Social Links</h2>
        Enter Link: <input style="width:70%; margin-bottom: 10px;" type="url" name="social_link_1" id="social_link_1"><br>
        Enter Link: <input style="width:70%; margin-bottom: 10px;" type="url" name="social_link_2" id="social_link_2"><br>
        Enter Link: <input style="width:70%; margin-bottom: 10px;" type="url" name="social_link_3" id="social_link_3"><br>
    </div>
...
// adding templates to display custom post

add_filter( 'template_include', 'social_img_template_function', 1 );



function social_img_template_function( $template_path ) {
    if ( get_post_type() == 'social_img' ) {
        if ( is_single() ) {
            include_once( plugin_dir_path( __FILE__ ) . 'single-social-img.php' );
        }
    }
}
